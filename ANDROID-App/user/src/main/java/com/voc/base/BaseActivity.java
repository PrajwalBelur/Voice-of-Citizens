...
import com.voc.util.ConnectivityChecker;
import com.voc.util.MySharedPreferences;
import com.voc.util.UIHandlers;
...
public class BaseActivity extends AppCompatActivity implements DisplayCommons {
    ...
    public final boolean notConnected() {
        ConnectivityChecker cc = new ConnectivityChecker(getApplicationContext());
        boolean isNotConnected = !cc.isConnected() && !cc.isConnectedWiFi();    // restrict to use only LAN connection
        if (isNotConnected) shortToast("Please turn on WiFi");
